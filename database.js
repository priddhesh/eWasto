...
const authenticate = async (phone, password, role) => {
  if (role === "user") {
    ...
    try {
      const [data] = await pool.query(
        `SELECT * FROM pickup_boys WHERE phone = ? AND password = ?`,
        [phone, password]
      );
...
const checkCode = async (code) => {
  try {
    const [data] = await pool.query(`SELECT * FROM users WHERE referral = ?`, [
      code,
    ]);
...
const creditPoints = async (user, points) => {
  try {
    const [data] = await pool.query(
      `SELECT credits FROM users WHERE name = ?`,
      [user]
    );
    let previousCredits = data[0].credits;

    await pool.query(`UPDATE  users SET credits = ? WHERE name = ?`, [
      previousCredits + points,
      user,
    ]);
  } catch (err) {
    console.log(err);
...
const checkCredits = async (referral, points) => {
  try {
    ...
    console.log(data[0]);
    await pool.query(`UPDATE  users SET credits = ? WHERE email = ?`, [
      previousCredits + points,
      email,
    ]);
  } catch (err) {
...
const userData = async (name, password) => {
  try {
    const [data] = await pool.query(
...
const getAdminEmail = async (uname, pwd) => {
  try {
    const [data] = await pool.query(
      `SELECT email FROM admin WHERE name = ? AND password = ?`,
      [uname, pwd]
    );
    return data;
  } catch (err) {
...
const pickupBoyDetails = async (phonePB) => {
  try {
    const [data] = await pool.execute(
      `SELECT * FROM pickup_boys WHERE phone = ?`,
      [phonePB]
    );
    return data;
  } catch (err) {
...
const getOrders = async (phone) => {
  try {
    const [data] = await pool.execute(
      `SELECT * FROM recycling_items WHERE phonePB = ? AND status = 0`,
      [phone]
    );
    return data;
  } catch (err) {
...
const getPhone = async (user, password) => {
  try {
    const [data] = await pool.execute(
      `SELECT phone FROM pickup_boys WHERE name = ? AND password = ?`,
      [user, password]
    );
...
const assignCredits = async (email, credits) => {
  try {
    ...
    await pool.execute(`UPDATE users SET credits = ? WHERE email = ?`, [
...
const confirmOrder = async (OTP, email, credits) => {
  try {
    await pool.execute(
      `UPDATE recycling_items SET status = 1 WHERE email = ? AND otp = ?`,
      [email, OTP]
    );

    const [data] = await pool.execute(
      `SELECT * FROM recycling_items WHERE status = 1 AND email = ? AND otp = ?`,
      [email, OTP]
    );
...
const userRequests = async (name, password) => {
  try {
    let [data] = await userData(name, password);
    let email = data.email;
    let [requests] = await pool.execute(
      `SELECT * FROM recycling_items WHERE email = ?`,
      [email]
    );
    return requests;
  } catch (err) {
